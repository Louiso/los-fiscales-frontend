...
function _get<T>(serverMock: any) {
  function _(url: string): FetchResponse<T> {
    const { match, endpointKey, query } = processUrl(`GET ${url}`, serverMock);
...
function _post<T>(serverMock: any) {
  function _(url: string, json: any): FetchResponse<T> {
    const { match, query, endpointKey } = processUrl(`POST ${url}`, serverMock);
...
function _put<T>(serverMock: any) {
  function _(url: string, json: any): FetchResponse<T> {
    const { match, query, endpointKey } = processUrl(`PUT ${url}`, serverMock);
...
function _delete<T>(serverMock: any) {
  function _(url: string): FetchResponse<T> {
    const { match, endpointKey, query } = processUrl(
      `DELETE ${url}`,
      serverMock
    );
